...
const terminalLoginUrl = () => window.TERMINAL_LOGIN_URL || '/terminal/login';
...
$('#advanced-search-btn').on('click', function() {
    ...
    $.getJSON(`${terminalBaseUrl()}/search`, params, function(res) {
        $('.loader').hide();
        ...
        $('.loader').hide();
        if (xhr.status === 401) {
            window.location.href = terminalLoginUrl();
            return;
        }
